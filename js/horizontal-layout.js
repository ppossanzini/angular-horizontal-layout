...
.directive("panel", ["$timeout", function ($timeout) {
  return {
    ...
    template: '<div class="panel-divider" ng-if="divider"><span>{{divider}}</span></div><div class="navigation-panel" ng-style="{width:currentSize}"><div class="navigation-panel-header"><div class="panel-title pull-left">{{title}}</div><div class="btn-group btn-group-xs pull-right square-borders"><button class="btn btn-sm" ng-click="changeSize(\'S\')" ng-if="canGrow"><i class="fa fa-compress"></i></button><button class="btn btn-sm" ng-click="changeSize(\'N\')" ng-if="canGrow"><i class="fa fa-television"></i></button><button class="btn btn-sm" ng-click="changeSize(\'B\')" ng-if="canGrow"><i class="fa fa-arrows-alt"></i></button><button class="btn btn-sm" ng-click="remove()" ng-if="canClose"><i class="fa fa-remove"></i></button></div></div><div class="navigation-panel-body" ng-transclude></div></div>',
